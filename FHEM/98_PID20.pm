# $Id: 98_PID20.pm 10722 2016-02-04 17:12:18Z john99sr $
####################################################################################################
#
#	98_PID20.pm
  ...
  while (1)
  {
    ...
    if ($actuationReq)
    {
      ...
      PID20_Log $hash, 5, 'actor:' . $hash->{helper}{actor} . ' actorCommand:' . $hash->{helper}{actorCommand}
        . ' actuation:' . $actuation;
      my $cmd = sprintf( "setreading %s %s %g", $hash->{helper}{actor}, $hash->{helper}{actorCommand}, $actuation ); # HB set -> setreading
